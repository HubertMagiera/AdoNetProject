...
namespace ProjectManagerBackend.Services
{
    public class UserService:IUserService
    {
        ...
        {
            //get user roles to map role name with role id
            ...
            string insert = "insert into user (user_name,user_surname,user_login,user_password,user_role_id) " +
                                    "values(@name,@surname,@login,md5(@password),@role)";
            ...
            if (dataSet.HasErrors)
            {
                ...
                throw new Exception("An error has occured during inserting new user");
            }
        ...
        public User LoginUser(string login, string password)
        {
            ...
            string query = "select u.user_id,u.user_name,u.user_surname,u.user_role_id,role.user_role_name " +
                                "from user as u " +
                                "inner join user_role as role on u.user_role_id = role.user_role_id " +
                                "where u.user_login = @login and u.user_password = md5(@password);";
